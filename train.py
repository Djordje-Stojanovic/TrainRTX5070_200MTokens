...
class GPT(nn.Module):
    ...
    def setup_optimizer(self, unembedding_lr=0.004, embedding_lr=0.2, matrix_lr=0.02,
        ...
        mup_output_lr_scale = MUP_BASE_WIDTH / model_dim  # Output layer: 1/width
        mup_muon_lr_scale = MUP_BASE_WIDTH / model_dim  # Hidden layers (Muon): 1/width
        print(f"muP LR scales (base={MUP_BASE_WIDTH}, width={model_dim}): "
              f"embed={mup_embed_lr_scale:.4f}, output={mup_output_lr_scale:.4f}, "
              f"muon={mup_muon_lr_scale:.4f}")
        param_groups = [
            ...
            dict(kind="adamw", params=value_emb_params, lr=embedding_lr * mup_embed_lr_scale, betas=adam_betas, eps=1e-10, weight_decay=0.1),
                ...
                for ci in range(0, len(group_params), muon_group_chunk):
                    ...
                    param_groups.append(
                        dict(
                            kind="muon",
                            params=chunk,
                            lr=matrix_lr * c_proj_lr_mult * mup_muon_lr_scale,
                            momentum=0.95,
                            ns_steps=5,
                            beta2=0.98,
                            weight_decay=weight_decay,
...
# muP reference width (hyperparameters are tuned at this width)
MUP_BASE_WIDTH = 768
...
def _run_training_once(runtime, tokenizer, config, device_batch_size, smoke_test):
    ...
    target_tokens = TOTAL_BATCH_SIZE * 3 if smoke_test else TOKEN_BUDGET
    ...
    def get_lr_multiplier(progress):
        ...
        if WARMDOWN_RATIO <= 0.0 or progress < 1.0 - WARMDOWN_RATIO:
            return 1.0
    ...
    while True:
        ...
        total_tokens_so_far = (step + 1) * TOTAL_BATCH_SIZE
        progress = min(total_tokens_so_far / max(target_tokens, 1), 1.0)
        ...
        if runtime.gpu_peak_flops:
            ...
            mfu_text = "n/a"
        remaining_tokens_M = max(0, target_tokens - total_tokens_so_far) / 1e6
        print(
            f"\rstep {step:05d} ({pct_done:.1f}%) | loss: {debiased_smooth_loss:.6f} | "
            f"lrm: {lrm:.2f} | dt: {dt*1000:.0f}ms | tok/sec: {tok_per_sec:,} | "
            f"mfu: {mfu_text} | epoch: {epoch} | remaining: {remaining_tokens_M:.1f}M tok    ",
            end="",
            flush=True,
        )
        ...
        if max_steps is not None and step >= max_steps:
            break
        if total_tokens_so_far >= target_tokens:
            break
